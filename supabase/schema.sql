...
-- Pools: owners and members can view
...
create policy "Users can view players in their pools" on players
  for select using (
    user_id = auth.uid()
    or id in (
      select pp.player_id from pool_players pp
      where pp.pool_id in (
        select pool_id from pool_players
        join players p on pool_players.player_id = p.id
        where p.user_id = auth.uid()
      )
    )
  );
